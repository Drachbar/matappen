...
export class ImageConverterService implements ImageProcessor {
  ...
  private downscaleImageProgressively(img: HTMLImageElement, targetWidth: number, targetHeight: number): HTMLCanvasElement {
    ...
    // Börja med originalstorlek
    canvas.width = img.width;
    canvas.height = img.height;
    ctx.drawImage(img, 0, 0);

    // Progressivt minska storleken
    while (canvas.width > targetWidth * 2 || canvas.height > targetHeight * 2) {
      canvas = this.downscaleStep(canvas, 0.5); // Skala ner 50% per steg
    }

    // Slutlig skalning till målstorlek
